...
action_endpoint = EndpointConfig(url="https://horo2105.herokuapp.com/webhook")
...
class MyNewInput(RasaChatInput):
    ...
    def blueprint(self, on_new_message):
        templates_folder = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'chatbot')
        ...
        def receive():
            ...
            if should_use_stream:
                return request.Response(self.stream_response(on_new_message, text,sender_id),content_type = 'text/event-stream')
            else:
...
input_channel = MyNewInput(url='https://horo2105.herokuapp.com')
